...
BOOL CreateBlankNewFormatIcon( LPICONIMAGE lpii )
{
    ...
    dwFinalSize += huys::PaletteSize( (LPSTR)&bmih );
    // and XOR bits
    ...
    lpii->lpbi = (LPBITMAPINFO)(lpii->lpBits);
    lpii->lpXOR = (LPBYTE)(lpii->lpbi) + sizeof(BITMAPINFOHEADER) + huys::PaletteSize( (LPSTR)&bmih );
...
BOOL MakeNewANDMaskBasedOnPoint( LPICONIMAGE lpIcon, POINT pt )
{
    ...
    memcpy( pXORBits, lpIcon->lpXOR, (lpIcon->lpbi->bmiHeader.biHeight) * huys::BytesPerLine((LPBITMAPINFOHEADER)(lpIcon->lpbi)) );
    ...
    memcpy( lpIcon->lpXOR, pXORBits, (lpIcon->lpbi->bmiHeader.biHeight) * huys::BytesPerLine((LPBITMAPINFOHEADER)(lpIcon->lpbi)) );
...
{
    return huys::WriteDIB( szFileName, (HANDLE)(LPBYTE)lpii->lpbi );
}
